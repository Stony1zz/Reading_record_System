...
//阅读记录添加
public class ReadInfoAddIFrame extends JInternalFrame {
	...
	public ReadInfoAddIFrame() {
		...
		lblNewLabel.setIcon(new ImageIcon(ReadInfoAddIFrame.class.getResource("/icon/readModiAndDel.jpg")));
	...
	class SaveButtonActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(getBookStyleNumber(ReadbookDao.selectRead()) == 1) {
				JOptionPane.showMessageDialog(null, "该阅读记录存在，请重新输入。");
				return;
			}
			...
			if(Bno.length() == 0) {
			 ...
			 int i = ReadbookDao.insertRead(Uno,Bno,Integer.parseInt(RpageTxt.getText().trim()));
